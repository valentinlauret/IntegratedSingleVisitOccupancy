...
out <- jags(win.data, params, "Aerial_RV.jags", inits = inits, n.chains = nc, n.thin = nt, n.iter = ni, n.burnin = nb, working.directory = getwd())

Aerial_RV <- out

save(Aerial_RV, file="RES_Aerial_RV.rdata")
...
load("RES_Aerial_RV.rdata")

head(Aerial_RV$BUGSoutput$summary)

# see posterior distribution of parameters

denplot(Aerial_RV, c("alpha.psi","beta.bathy", "beta.sst"))

# see parameters values for saved iterations

traplot(Aerial_RV,"beta.bathy")
traplot(Aerial_RV,"beta.sst")
traplot(Aerial_RV,"beta.eff")