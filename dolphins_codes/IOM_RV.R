...
# Call JAGS from R , !!! It takes a long time !!!

out <- jags(win.data, params, "IOM_RV.jags", inits = inits, n.chains = nc, n.thin = nt, n.iter = ni, n.burnin = nb, working.directory = getwd())

IOM_RV <- out

save(IOM_RV, file="RES_IOM_RV.rdata")
...
load("RES_IOM_RV.rdata")

head(IOM_RV$BUGSoutput$summary)

# see posterior distribution of parameters

denplot(IOM_RV, c("alpha.psi","beta.bathy", "beta.sst"))

# see parameters values for saved iterations

traplot(IOM_RV,"beta.bathy")
traplot(IOM_RV,"beta.sst")
traplot(IOM_RV,"beta.eff")