...
out <- jags(win.data, params, "IOM_SV.jags", inits = inits, n.chains = nc, n.thin = nt, n.iter = ni, n.burnin = nb, working.directory = getwd())

IOM_SV <- out

save(IOM_SV, file="RES_IOM_SV.rdata")
...
load("RES_IOM_SV.rdata")

head(IOM_SV$BUGSoutput$summary)

# see posterior distribution of parameters

denplot(IOM_SV, c("alpha.psi","beta.bathy", "beta.sst"))

# see parameters values for saved iterations

traplot(IOM_SV,"beta.bathy")
traplot(IOM_SV,"beta.sst")
traplot(IOM_SV,"beta.eff")